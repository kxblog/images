		...
		<div id="respond" class="comment-respond wow fadeInUp ms bk da">
			...
			<?php else : ?>
				<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
						...
						<div class="comment-user-inf">
							<div class="user-avatar">
								<?php 
								 ...
								 ?>
								<img alt="匿名" src="<?php echo $random_avata[$random_avata_array]; ?>">
							</div>
							...
							<div class="user-avatar">
								<?php global $current_user;wp_get_current_user();
									if (zm_get_option('cache_avatar')) {
										...
										echo get_avatar( $current_user->user_email, 96, '', $user_identity );
									}
								?>
							</div>
							<div class="comment-user-inc">
						...
						<div class="comment-user-inf">
							<div class="user-avatar">
								<?php if (zm_get_option('cache_avatar')) { ?>
									<?php echo begin_avatar($comment_author_email, $size = '96', '', $comment_author );  ?>
								<?php } else { ?>
									<?php echo get_avatar($comment_author_email, $size = '96', '', $comment_author );  ?>
								<?php } ?>
							</div>
							<div class="comment-user-inc">
								<h3 id="reply-title" class="comment-reply-title">
					...
					<?php if ( ! is_user_logged_in() ): ?>
						<div id="comment-author-info" class="comment-info<?php if (zm_get_option('not_comment_form')) { ?> author-form<?php } ?>">
							<p class="comment-form-author pcd">
								<label class="bk da" for="author"><?php _e( '昵称', 'begin' ); ?><span class="required bgt"><?php if ($req) echo "*"; ?></span></label>
							...
							<?php if ( zm_get_option('no_email') == '' ) { ?>
								<p class="comment-form-email pcd">
									<label class="bk da" for="email"><?php _e( '邮箱', 'begin' ); ?><span class="required bgt"><?php if ($req) echo "*"; ?></span></label>
					...
					<?php if (zm_get_option('emoji_show')) { ?><div class="clear"></div><p class="emoji-box"><?php get_template_part( 'inc/smiley' ); ?></p><?php } ?>
					<p class="comment-form-comment"><textarea id="comment" class="da" name="comment" rows="4" tabindex="1" placeholder="<?php echo stripslashes( zm_get_option('comment_hint') ); ?>" onfocus="this.placeholder=''" onblur="this.placeholder='<?php echo stripslashes( zm_get_option('comment_hint') ); ?>'"></textarea></p>
